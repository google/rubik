...
class MerchantCenterUpdaterDoFn(beam.DoFn):
    ...
    def process(self, batch: Tuple[str, List[RubikOffer]], **kwargs):
        """
        From apache beam, process method is executed
           see: https://beam.apache.org/documentation/programming-guide/
        Args:
            batch (Tuple[str, List[RubikOffer]]): A batch is defined by
            [merchant_id, list of rubik offers]

        #TODO Improvements:
            - Create separate methods for: defining entries, request and output
        """
        try:
            ...
            request = (
                self._get_merchant_center_service()
                .products()
                .custombatch(body=request_body)
            )
            ...
            if result["kind"] == "content#productsCustomBatchResponse":
                ...
                for entry in entries:
                    ...
                    if product:
                        ...
                        logger.error(
                            json.dumps(
                                errors, sort_keys=True, indent=2, separators=(",", ": ")
                            )
                        )
            else:
